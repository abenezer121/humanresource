...
usersController.getSingleUser = async (req, res) => {
  ...
  try {
    ...
    user = await Users.Users.findOne({ _id: _id });
    res.status(200).send({
      code: 200,
      message: 'Successful',
      data: user
...
usersController.registerUser = async (req, res) => {
  try {
    ...
    body.password = hash;
    const user = new Users.Users(body);
...
usersController.loginUser = async (req, res) => {
    try {
        ...
        const result = await Users.Users.findOne({ email: email });
        if (!result) {
...
usersController.getNextId = async (req, res) => {
  try {
    const max_result = await Users.Users.aggregate([
...
usersController.deleteUser = async (req, res) => {
  ...
  try {
    ...
    const result = await Users.Users.findOneAndDelete({
      _id: _id
    });
...
async function runUpdate(_id, updates, res) {
  try {
    const result = await Users.Users.updateOne(
      {
        _id: _id
      },
      {
